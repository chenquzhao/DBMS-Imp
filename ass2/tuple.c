...
Bits tupleHash(Reln r, Tuple t)
{
	...
	assert(vals != NULL);
	tupleVals(t, vals);
        //calc hash for each val 
        Bits *val_hashs = malloc(nvals*sizeof(Bits));
	for (i = 0; i < nvals; i++) {
                val_hashs[i] = hash_any((unsigned char *)vals[i],strlen(vals[i]));
	}
        //calc multi-attribute hash by using choice vector 
        Bits hash=0;
	for (i = 0; i < MAXBITS; i++) {
            if( bitIsSet(val_hashs[chvec(r)[i].att],chvec(r)[i].bit)){
                hash=setBit(hash,i);
            }
	}     
        bitsString(hash,buf);
