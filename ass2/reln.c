...
int pageCapacity(Reln r)
{
    return (int)floor(1024 / (10 * r->nattrs));
}
...
Offset pageFree(Page p);
Tuple PageGetTuple(Page p, Offset os);

PageID addToRelation(Reln r, Tuple t)
{
	Bits h, p;
	
	if(r->ntups % pageCapacity(r) == 0 && r->ntups != 0)
	{
		//split the page
		PageID split_pid = r->sp + (1<<(r->depth));
		...
		Page original = getPage(r->data, r->sp);
    	Page new = newPage(); 
    	pageSetOvflow(new, pageOvflow(original));

    	Page split_page = getPage(r->data, split_pid);

		while (os < pageFree(original)) 
		{
            Tuple t = PageGetTuple(original, os);
            ...
            else if (np == split_pid)
			{
				addToPage(split_page, t);
			}

            os += tupLength(t)+1;
    	}

	putPage(r->data, r->sp, new);
    putPage(r->data, split_pid, split_page);

	//split overflow page
	if (pageOvflow(original) != NO_PAGE) 
		{
            PageID ov_pid = pageOvflow(original);          
            original = getPage(r->ovflow, ov_pid);

            new = getPage(r->data, r->sp);
            split_page = getPage(r->data, split_pid);
